...
int main()
{
	...
	Device device{ physical_devices[0], window.get_surface() };
	auto queue_family_properties = device.get_physical_device_queue_family_properties();

	Swapchain swapchain{ device, window.get_surface(), width, height };
	...
	std::shared_ptr<RenderPassBuilder> render_pass_builder = RenderPassBuilder::create();
	...
	RenderPass render_pass{ device, render_pass_builder };
	...
	Buffer vbo{ device, vk::BufferUsageFlagBits::eVertexBuffer, geometry.get_packed_vertex_attributes() };
	Buffer ibo{ device, vk::BufferUsageFlagBits::eIndexBuffer, geometry.get_indices() };
	Buffer ubo{ device, vk::BufferUsageFlagBits::eUniformBuffer, sizeof(UniformBufferData), nullptr };
	...
	auto pipeline_options = GraphicsPipeline::Options()
		...
		.enable_depth_test()
		.samples(msaa);
	GraphicsPipeline pipeline{ device, render_pass, pipeline_options };
	 ...
	 *
	 ***********************************************************************************/
	Image image_ms{ device,
		vk::ImageType::e2D,
		vk::ImageUsageFlagBits::eTransientAttachment | vk::ImageUsageFlagBits::eColorAttachment,
		swapchain_format, fs_extent, 1, 1,
		vk::ImageTiling::eOptimal, msaa };

	ImageView image_ms_view{ device, image_ms };

	Image image_depth{ device,
		vk::ImageType::e2D,
		vk::ImageUsageFlagBits::eDepthStencilAttachment,
		device.get_supported_depth_format(), fs_extent, 1, 1,
		vk::ImageTiling::eOptimal, msaa };

	ImageView image_depth_view{ device, image_depth, vk::ImageViewType::e2D, Image::build_single_layer_subresource(vk::ImageAspectFlagBits::eDepth) };

	Image image_sdf_map{ device,
		vk::ImageType::e3D,
		vk::ImageUsageFlagBits::eSampled | vk::ImageUsageFlagBits::eTransferDst,
		swapchain_format, { width, height, 32 }, 1, 1,
		vk::ImageTiling::eOptimal };

	ImageView image_sdf_map_view{ device, image_sdf_map, vk::ImageViewType::e3D };	// by default, ImageView's are 2D.
	 ...
	 *
	 ***********************************************************************************/
	CommandPool command_pool{ device, QueueType::GRAPHICS };
	CommandBuffer temp_command_buffer{ device, command_pool };
	...
	for (size_t i = 0; i < swapchain_image_views.size(); ++i)
	{
		...
		Framebuffer fb{ device, render_pass, name_to_image_view_map, width, height };
	...
	*
	***********************************************************************************/
	Semaphore image_available_sem{ device };
	Semaphore render_complete_sem{ device };
